const ArgumentType = require('../../extension-support/argument-type');
const BlockType = require('../../extension-support/block-type');
const Cast = require('../../util/cast');
const log = require('../../util/log');
const formatMessage = require('format-message');  //多言語化のために必要

//ブロックに付けるアイコン
const blockIconURI = 'data:image/svg+xml;base64,';

//メニューに付けるアイコン
const menuIconURI = 'data:image/svg+xml;base64,';

//メニューで使う配列
const Kanirobo2MenuSetGpio = {
    FORWARD: '1',
    BACKWARD:  '0'
}
const Kanirobo2MenuGpio = {
    ONE: "25",     //数字の場合も「文字列」扱いしないとエラーが出る
    TWO: "32"
}
const Kanirobo2Menu3 = {
    ENABLE: 'on',
    DISABLE:  'off'
}
const Kanirobo2MenuLightSensor = {
    FIRST: "36",
    SECOND: "34",
    THIRD: "35",
    FOURTH: "2"
}
const Kanirobo2MenuPwm = {
    ONE: "26",
    TWO: "33"
}
const Kanirobo2MenuServo = {
    ONE: "27",
    TWO: "14"
}

//クラス定義
class Kanirobo2 {
    constructor (runtime) {
        /**
         * The runtime instantiating this block package.
         * @type {Runtime}
         */
        this.runtime = runtime;

        //this._onTargetCreated = this._onTargetCreated.bind(this);
        //this.runtime.on('targetWasCreated', this._onTargetCreated);
    }

    //ドロップボックスメニュー (menu_set_gpio) 
    static get Kanirobo2MenuSetGpio () {
        return Kanirobo2MenuSetGpio;
    }
    get MENU_SET_GPIO () {
        return [
            {
                text: formatMessage({
                    id: 'kanirobo2.MenuSetGpio.forward',
                    default: 'forward',
                }),
                value: Kanirobo2MenuSetGpio.FORWARD
            },
            {
                text: formatMessage({
                    id: 'kanirobo2.MenuSetGpio.backward',
                    default: 'backward',
                }),
                value: Kanirobo2MenuSetGpio.BACKWARD
            }
        ];
    }

    //ドロップボックスメニュー (MenuGpio) 
    static get Kanirobo2MenuGpio () {
        return Kanirobo2MenuGpio;
    }
    get MENU_GPIO () {
        return [
            {
                text: '1',
                value: Kanirobo2MenuGpio.ONE
            },
            {
                text: '2',
                value: Kanirobo2MenuGpio.TWO
            }
        ];
    }

    //ドロップボックスメニュー (Menu3) 
    static get Kanirobo2Menu3 () {
        return Kanirobo2Menu3;
    }
    get MENU3 () {
        return [
            {
                text: formatMessage({
                    id: 'kanirobo2.Menu3.enable',
                    default: 'enable',
                }),
                value: Kanirobo2Menu3.ENABLE
            },
            {
                text: formatMessage({
                    id: 'kanirobo2.Menu3.disable',
                    default: 'disable',
                }),
                value: Kanirobo2Menu3.DISABLE
            }
        ];
    }

    //ドロップボックスメニュー (MenuLightSensor) 
    static get Kanirobo2MenuLightSensor () {
        return Kanirobo2MenuLightSensor;
    }
    get MENU_LIGHT_SENSOR () {
        return [
            {
                text: '1',
                value: Kanirobo2MenuLightSensor.FIRST
            },
            {
                text: '2',
                value: Kanirobo2MenuLightSensor.SECOND
            },
            {
                text: '3',
                value: Kanirobo2MenuLightSensor.THIRD
            },
            {
                text: '4',
                value: Kanirobo2MenuLightSensor.FOURTH
            }
        ];
    }

    static get Kanirobo2MenuPwm () {
        return Kanirobo2MenuPwm;
    }
    get MENU_PWM () {
        return [
            {
                text: '1',
                value: Kanirobo2MenuPwm.ONE
            },
            {
                text: '2',
                value: Kanirobo2MenuPwm.TWO
            }
        ];
    }

    static get Kanirobo2MenuServo () {
        return Kanirobo2MenuServo;
    }
    get MENU_SERVO () {
        return [
            {
                text: '1',
                value: Kanirobo2MenuServo.ONE
            },
            {
                text: '2',
                value: Kanirobo2MenuServo.TWO
            }
        ];
    }


    //ブロック定義
    getInfo () {
        return {
            id: 'kanirobo2',
            name: formatMessage({
                id: 'kanirobo2.name',
                default: 'Kanirobo2'
            }),
            menuIconURI: menuIconURI,
            blockIconURI: blockIconURI,
            blocks: [
                {
                    opcode: 'ResetGpio',
                    text: formatMessage({
                        id: 'kanirobo2.ResetGpio',
                        default: 'Initialize motor [TEXT] GPIO ',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_gpio',
                            defaultValue: Kanirobo2MenuGpio.ONE
			}
                    }
                },
                {
                    opcode: 'ResetPwm',
                    text: formatMessage({
                        id: 'kanirobo2.ResetPwm',
                        default: 'Initialize motor [TEXT] PWM',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_pwm',
                            defaultValue: Kanirobo2MenuPwm.ONE
			}
                    }
                },
                {
                    opcode: 'SetGpio',
                    text: formatMessage({
                        id: 'kanirobo2.SetGpio',
                        default: 'set motor [TEXT1] GPIO [TEXT2]',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT1: {
                            type: ArgumentType.STRING,
                            menu: 'menu_gpio',
                            defaultValue: Kanirobo2MenuGpio.ONE
			},
                        TEXT2: {
                            type: ArgumentType.STRING,
                            menu: 'menu_set_gpio',
                            defaultValue: Kanirobo2MenuSetGpio.FORWARD
                        }
                    }
                },
                {
                    opcode: 'SetPwm',
                    text: formatMessage({
                        id: 'kanirobo2.SetPwm',
                        default: 'set motor [TEXT] duty [NUM] %',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_pmw',
                            defaultValue: Kanirobo2MenuPwm.ONE
			},
                        NUM: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 100
                        }
                    }
                },
                {
                    opcode: 'ResetLightSensor',
                    text: formatMessage({
                        id: 'kanirobo2.ResetLightSensor',
                        default: 'Initialize light sensor [TEXT]',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_light_sensor',
                            defaultValue: Kanirobo2MenuLightSensor.FIRST
			}
                    }
                },
                {
                    opcode: 'LightSensorValue',
                    text: formatMessage({
                        id: 'kanirobo2.LightSensorValue',
                        default: 'light sensor [TEXT] value'
                    }),		    
                    blockType: BlockType.REPORTER,
                    arguments: {
			TEXT: {
			    type: ArgumentType.STRING,
			    menu: 'menu_rihgt_sensor',
                            defaultValue: Kanirobo2MenuLightSensor.FIRST
			}
                    }
                },
                {
                    opcode: 'ResetServoMotor',
                    text: formatMessage({
                        id: 'kanirobo2.ResetServoMotor',
                        default: 'Initialize servo motor [TEXT] (period:[NUM] ms)',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_servo',
                            defaultValue: Kanirobo2MenuServo.ONE
			},
                        NUM: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 20
                        }			
                    }
                },	
                {
                    opcode: 'SetServoMotorPulseWigth',
                    text: formatMessage({
                        id: 'kanirobo2.SetServoMotorPluseWigth',
                        default: 'Set servo motor [TEXT] pulse width [NUM] ms',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'menu_servo',
                            defaultValue: Kanirobo2MenuServo.ONE
			},
                        NUM: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 1
                        }
                    }
                }
/*
                {
                    opcode: 'command9',
                    text: formatMessage({
                        id: 'kanirobo2.command9',
                        default: 'Set servo motor [TEXT] duty [NUM]',
                    }),		    		    
                    blockType: BlockType.COMMAND,
                    arguments: {
			TEXT: {
                            type: ArgumentType.STRING,
                            menu: 'Menu_Servo',
                            defaultValue: Kanirobo2Menu_Servo.LIGHT
			},
                        NUM: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        }
                    }
                },
                {
                    opcode: 'value1',
                    text: formatMessage({
                        id: 'kanirobo2.value1',
                        default: 'angle [NUM]'
                    }),		    
                    blockType: BlockType.REPORTER,
                    arguments: {
                        NUM: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 0
                        }  
                    }
                }
*/
            ],
	    //ドロップボックスメニューを使う場合は以下に定義が必要
            menus: {
                menu_set_gpio: {
                    acceptReporters: false,
                    items: this.MENU_SET_GPIO
                },
                menu_gpio: {
                    acceptReporters: false,
                    items: this.MENU_GPIO
                },
                menu3: {
                    acceptReporters: false,
                    items: this.MENU3
                },
                menu_rihgt_sensor: {
                    acceptReporters: false,
                    items: this.MENU_LIGHT_SENSOR
                },
                menu_pwm: {
                    acceptReporters: false,
                    items: this.MENU_PWM
                },
                menu_servo: {
                    acceptReporters: false,
                    items: this.MENU_SERVO
                },
            }
        };
    }

    // navigator.userAgent ブロック．入力なし
    navigator_userAgent () {
        return navigator.userAgent;
    }
    
    command1 (args) { 
        const text = Cast.toString(args.TEXT);
        log.log(text);
    }

    reset_gpio (args) { 
        const text = Cast.toString(args.TEXT);
        log.log(text);
    }

    reset_pwm (args) { 
        const text = Cast.toString(args.TEXT);
        log.log(text);
    }

    set_gpio (args) { 
        const text1 = Cast.toString(args.TEXT1);
        const text2  = Cast.toString(args.TEXT2); 
        log.log(text1);
        log.log(text2);
    }

    set_pwm (args) {
        const text = Cast.toString(args.TEXT);
        const num  = Cast.toString(args.NUM);
        log.log(text);
        log.log(num);
    }

    reset_light_sensor (args) { 
        const text = Cast.toString(args.TEXT);
        log.log(text);
    }

    // value0 
    light_sensor_value (args) {
        const text = Cast.toString(args.TEXT);
        log.log(text);
    }

    reset_servo_motor (args) { 
        const text = Cast.toString(args.TEXT);
	const num  = Cast.toString(args.NUM);
        log.log(text);
        log.log(num);	
    }

    set_servo_motor_pulse_wigth (args) {
        const text = Cast.toString(args.TEXT);
        const num  = Cast.toString(args.NUM);
        log.log(text);
        log.log(num);
    }
/*
    command9 (args) {
        const text = Cast.toString(args.TEXT);
        const num  = Cast.toString(args.NUM);
        log.log(text);
        log.log(num);
    }

    value1 () {
        const num  = Cast.toString(args.NUM);
        log.log(num);
    }
*/	
}

module.exports = Kanirobo2
